document.addEventListener("DOMContentLoaded", () => {
  ...
  $("[data-start-guide]")?.addEventListener("click", () => {
    $("#before-start")?.scrollIntoView({ behavior: "smooth" });
  });
  ...
  function setFirstRowStartWeight(startWeight) {
    ...
    const weightInput = firstRow ? $('input[name="vekt[]"]', firstRow) : null;
  ...
  function updateRowNumbers() {
    $$('[data-row]').forEach((row, index) => {
      ...
      if (setInput && index === 0 && setInput.value.trim() === "") {
        setInput.value = "1";
      }

      if (repsInput) {
        repsInput.placeholder = index === 0 ? "8 til 10" : "Reps";
      }
    });
  }
  ...
  trainingForm?.addEventListener("input", (event) => {
    ...
    const firstRow = $('[data-row]');
    if (firstRow && firstRow.contains(weightInput)) {
  ...
  trainingForm?.addEventListener("click", (event) => {
    ...
    if (addButton) {
      ...
      const newRow = createRowFromFirstSet();
      if (currentRow && newRow) currentRow.after(newRow);
      updateRowNumbers();
      startRestTimer(180);
      return;
    }
  ...
  function collectRows() {
    return $$('[data-row]').map((row, index) => {
      const weight = $('input[name="vekt[]"]', row)?.value || "ikke fylt ut";
      const sets = $('input[name="sett[]"]', row)?.value || "ikke fylt ut";
      const reps = $('input[name="repetisjoner[]"]', row)?.value || "ikke fylt ut";

      return `${index + 1}. Vekt: ${weight} kg\n   Sett: ${sets}\n   Repetisjoner: ${reps}`;
    });
  }
