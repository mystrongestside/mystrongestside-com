document.addEventListener("DOMContentLoaded", () => {
  ...
  completeSessionInput?.addEventListener("change", () => {
    const isComplete = completeSessionInput.checked;

    if (completionConfirmation) {
      completionConfirmation.hidden = !isComplete;
      completionConfirmation.classList.toggle("fade-in-complete", isComplete);
    }

    if (copyActions) {
      copyActions.hidden = !isComplete;
      copyActions.classList.toggle("fade-in-step", isComplete);
    }

    if (nextExercise) {
      nextExercise.hidden = !isComplete;
      nextExercise.classList.toggle("fade-in-next", isComplete);
